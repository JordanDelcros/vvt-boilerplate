...
export default function( source ){
	...
	return {
		info: `🔊 pack-sound ${ packName }`,
		action: async () => {

			if( !AssetsPacker.FFMpegAvailable ){

				console.log("⚠️ FFMpeg (FFProbe) not available!");
				return source;
			...
			await execAsync(`ffmpeg -y ${ inputs.join(" ") } -filter_complex ${ filter } -map "[outa]" -c:a libopus -b:a 96K -f webm ${ source.generated.prefer }`);
